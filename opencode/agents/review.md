...
Also ensure that all conventions (listed below) and linting/formatting rules are
followed. In most projects, you can simply run `make check` to run formatters, linters
and type checkers. If this doesn't work, then you can code formatters with `uv run ruff
format`, linters with `uv run ruff check --fix` and type checkers with `uv run pyrefly
check`.
