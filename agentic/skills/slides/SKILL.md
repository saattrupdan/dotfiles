...
- **Mark anything that shouldn't be translated with `translate="no"`.** The browser's built-in translation otherwise garbles content that isn't prose: emojis turn into mojibake, and identifiers get mistranslated (e.g. `alexandra.dk` → `alelandra.dk`). So: wrap every literal emoji or pictographic glyph (e.g. `<span class="task-emoji" translate="no">💬</span>`), and add `translate="no"` to proper nouns and identifiers — email addresses, URLs/handles, person names, brand/affiliation names, code. The template's JS already auto-marks the `speaker`, `email`, and `contact-label` `[data-fill]` spans plus any `.affiliation` element, so reuse those hooks; only add markers manually for new fixed text of this kind. Inline SVG icons are unaffected and need no marker.
