...
- **Wrap every emoji or icon glyph in `translate="no"`.** Any literal emoji or pictographic character rendered as text (e.g. `<span class="task-emoji" translate="no">💬</span>`) must carry `translate="no"`, otherwise the browser's built-in translation garbles it into mojibake when a visitor translates the deck. Inline SVG icons are unaffected and need no marker.
