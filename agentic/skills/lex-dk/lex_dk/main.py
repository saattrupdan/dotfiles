...
def main() -> None:
    p = argparse.ArgumentParser(description="Anonymous lex.dk endpoints (CLI).")
    p.add_argument(
        "--host",
    ...
    ac = sub.add_parser(
        "autocomplete", help="Free-text autocomplete (returns up to 5 hits)."
    )
    ac.add_argument("term", help="Search term, e.g. 'Marie Curie'.")
    ac.add_argument("--raw", action="store_true", help="Print raw JSON.")
    ac.set_defaults(func=cmd_autocomplete)

    ar = sub.add_parser("article", help="Fetch /<Slug>.json - full article as JSON.")
    ar.add_argument("slug", help="Article slug, e.g. Marie_Curie (no leading slash).")
    ar.add_argument("--raw", action="store_true", help="One-line JSON.")
    ar.set_defaults(func=cmd_article)

    tx = sub.add_parser(
        "taxonomy", help="Fetch /.taxonomy/<id>.json - node breadcrumb."
    )
    tx.add_argument("id", help="Taxonomy ID, e.g. 1648.")
    tx.add_argument("--raw", action="store_true", help="Print raw JSON.")
    tx.set_defaults(func=cmd_taxonomy)

    rc = sub.add_parser(
        "recent", help="Fetch /.recent-activities.json - recent edits feed."
    )
    rc.add_argument(
        "--limit", type=int, default=20, help="Max items to print (default 20)."
    )
    rc.add_argument("--raw", action="store_true", help="Print raw JSON.")
    rc.set_defaults(func=cmd_recent)

    df = sub.add_parser(
        "define",
        help="Den Danske Ordbog passthrough at /api/definition/v1/definition/<word>.",
    )
    df.add_argument("word", help="Danish word, e.g. blod.")
    df.add_argument("--raw", action="store_true", help="Print raw JSON.")
    df.set_defaults(func=cmd_define)

    st = sub.add_parser("status", help="GET /.status - plaintext health check.")
    st.set_defaults(func=cmd_status)

    an = sub.add_parser("announcements", help="Site-wide announcement banners.")
    an.add_argument("--raw", action="store_true", help="Print raw JSON.")
    an.set_defaults(func=cmd_announcements)

    sm = sub.add_parser("sitemap", help="List sitemap shards from the sitemap index.")
    sm.add_argument("--raw", action="store_true", help="Print as JSON list.")
    sm.set_defaults(func=cmd_sitemap)

    ur = sub.add_parser("urls", help="Print all <loc> URLs in a sitemap shard.")
    ur.add_argument(
        "--shard",
        required=True,
        help="Shard number (e.g. 1) or full sitemap URL.",
    )
    ur.add_argument("--grep", help="Regex (case-insensitive) to filter URLs.")
    ur.add_argument("--raw", action="store_true", help="Print as JSON list.")
    ur.set_defaults(func=cmd_urls)

    am = sub.add_parser(
        "article-meta",
        help="Fetch /<Slug> and extract articleId, breadcrumb, schema.org JSON-LD.",
    )
    am.add_argument("slug", help="Article slug, e.g. Marie_Curie (no leading slash).")
    am.add_argument("--raw", action="store_true", help="One-line JSON.")
    ...
    try:
        ...
        if body:
            if ctype.startswith(
                ...
                sys.stderr.write(body.rstrip() + "\n")
            else:
                sys.stderr.write(body[:160].replace("\n", " ").rstrip() + " ...\n")
        sys.exit(2)
...
def cmd_autocomplete(
    ...
    if not args.term.strip():
        sys.stderr.write("autocomplete: empty query\n")
        sys.exit(2)
    qs = urllib.parse.urlencode({"query": args.term})
    text, _ = _request(args.host, f"/.search/autocomplete?{qs}")
    try:
...
def cmd_article(args: argparse.Namespace) -> None:
    ...
    slug = urllib.parse.quote(args.slug.lstrip("/"), safe="_-,.()%")
    ...
    print(f"subject:      {obj.get('subject_title')}  ({obj.get('subject_url')})")
    ...
    if authors:
        ...
        for a in authors:
            ...
            ident = a.get("id")
            print(f"  - {name}" + (f" (id={ident})" if ident else ""))
...
def cmd_taxonomy(args: argparse.Namespace) -> None:
    text, _ = _request(args.host, f"/.taxonomy/{args.id}.json")
    obj = json.loads(text)
    if args.raw:
        _print_json(obj, raw=True)
        return
...
def cmd_recent(args: argparse.Namespace) -> None:
    text, _ = _request(args.host, "/.recent-activities.json")
    items = json.loads(text)
    if args.raw:
        _print_json(items, raw=True)
        return
...
def cmd_define(args: argparse.Namespace) -> None:
    ...
    text, _ = _request(args.host, f"/api/definition/v1/definition/{word}")
    obj = json.loads(text)
    if args.raw:
        _print_json(obj, raw=True)
        return
    if not obj:
        print(f"(no definition for {args.word!r})")
        return
    for entry in obj:
...
def cmd_announcements(
    ...
    text, _ = _request(args.host, "/.announcements?v=2")
    obj = json.loads(text)
...
def cmd_urls(args: argparse.Namespace) -> None:
    ...
    for loc in locs:
        if loc == args.shard or loc.endswith(f"/sitemap{args.shard}.xml"):
            target = loc
            break
    if target is None:
        sys.stderr.write(f"Shard {args.shard!r} not found. Available:\n")
        for loc in locs:
...
def cmd_article_meta(
    ...
    path = "/" + urllib.parse.quote(args.slug.lstrip("/"), safe="_-,.()%")
    text, _ = _request(
        args.host,
        path,
        accept="text/html",
    ...
    if m:
        try:
            ...
            if dl and isinstance(dl, list):
                out["articleId"] = dl[0].get("articleId")
                out["breadcrumb"] = dl[0].get("breadcrumb")
        except json.JSONDecodeError:
            pass
