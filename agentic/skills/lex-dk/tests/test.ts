...
const SKILL_NAME = "lex-dk";

/**
 * Test cases — direct CLI commands to test the lex-dk skill
 * Tests search, article retrieval, and help
 */
const TEST_CASES: Array<{ name: string; cmd: string; expectMinResults: number }> = [
  // Basic help check
  { name: "Help command", cmd: "lexdk --help", expectMinResults: 1 },
  
  // Search/autocomplete tests
  { name: "Autocomplete demokrati", cmd: "lexdk autocomplete \"demokrati\"", expectMinResults: 1 },
  { name: "Autocomplete Marie Curie", cmd: "lexdk autocomplete \"Marie Curie\"", expectMinResults: 1 },
  
  // Article retrieval tests
  { name: "Article demokrati", cmd: "lexdk article demokrati", expectMinResults: 1 },
  { name: "Article Marie_Curie", cmd: "lexdk article Marie_Curie", expectMinResults: 1 },
];
...
async function preflightChecks(): Promise<{ ok: boolean; errors: string[] }> {
  ...
  try {
    await $`which lexdk`.quiet();
  } catch {
    errors.push("lexdk CLI not found on PATH");
  }
  
  // Check CLI responds to --help
  try {
    await $`lexdk --help`.quiet();
  } catch {
    errors.push("lexdk CLI failed to respond to --help");
  }
  
  // Python linting — format check
  try {
    await $`cd ${SKILL_DIR} && uv run ruff format --check lex_dk/`.quiet();
  } catch {
  ...
  try {
    await $`cd ${SKILL_DIR} && uv run ruff check lex_dk/`.quiet();
  } catch {
