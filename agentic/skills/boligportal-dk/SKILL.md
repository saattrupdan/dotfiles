...
### Search listings `POST /api/listing/listings/` (anonymous)

Backs the main listing hub pages. Returns paginated listing data.
...
### Map listings `POST /api/search/map` (anonymous)

Same filter shape as `/api/listing/listings/`. Returns listings with `lat`/`lng` coordinates for map pins.
...
### Top favorites `POST /api/listing/top-favorite-ads/` (anonymous)

Most-favorited listings across all users. Request: `{"limit": 10}`.
