...
- boligportal's `search`/`top-favorites` endpoints are session-bound; `promoted` works anonymously.
...
- **`bolig rent search` returns an auth error** — the listings endpoint is session-bound; use `bolig rent promoted` for anonymous results.
