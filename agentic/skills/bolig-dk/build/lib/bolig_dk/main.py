    ...
    while len(results) < args.limit:
        ...
        if args.keyword:
            ...
            with ThreadPoolExecutor(max_workers=args.workers) as pool:
                futures = {
                    pool.submit(_rent_fetch_detail, url, cache, ttl_days, use_cache): (
                        item,
                        url,
                    )
                ...
                for future in as_completed(futures):
                    ...
                    if (
                        _matches(haystack, args.keyword, args.match)
...
def _add_rent_filters(p: argparse.ArgumentParser) -> None:
    ...
    p.add_argument(
        ...
        type=int,
        default=18,
        help="max results to return (default 18); fetches multiple pages as needed",
    )
...
def _add_buy_common(p: argparse.ArgumentParser) -> None:
    """Add the shared boligsiden sort/raw options to a parser."""
    p.add_argument(
        "--limit",
        type=int,
        default=20,
        help="max results to return (default 20); fetches multiple pages as needed",
    )
