/**
 * `/solo <prompt>` — disable the orchestrator lockdown for the rest of
 * the session, allowing direct tool calls (bash, read, write, edit,
 ...
 * continue to bypass the lockdown.
 */
...
const BANNER =
	"SOLO MODE: the orchestrator lockdown is disabled for the rest of this " +
	"session (until `/no-solo`). You may call any tool directly (bash, read, " +
	"write, edit, etc.) without delegating to a subagent. Run the user's " +
	"request directly — do not wrap it in a planner → builder flow unless the " +
	"user specifically asks for one.";

export default function (pi: ExtensionAPI) {
	pi.registerCommand("solo", {
		description:
			"Disable orchestrator lockdown for the rest of the session (until /no-solo).",
