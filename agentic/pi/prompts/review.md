---
description:
  One reviewer audit; builders fix only substantive defects without re-review.
---
...
1. **Review.** Call the `subagent` tool with `agent: "reviewer"`, a 1–5-word
   ...
   provided), use this default task: "Audit the implementation of ABC in commits XYZ
   and return a verdict (LGTM / LGTM with nits / Needs changes / Block) with findings."
   Here ABC is the implemented task and XYZ is a list of commit hashes. Pass the
   argument to the reviewer to scope the audit.
...
3. **Build (if needed).** If the verdict is "Needs changes", treat its substantive
   ...
   issues. Do not call the reviewer again after the fixes; the user can request a
   separate review if wanted. Ask the user only when a fix requires a material
   user-level decision or permission under the questions-and-autonomy policy.
4. **Report.** Summarise the review verdict and findings. If changes were made, include
   the builder's commit subject and note that the fixes were not re-reviewed.
...
`builder`(s) after a "Needs changes" verdict, and never review more than once per
command.
