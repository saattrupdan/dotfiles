...
2. **Build (if needed).** If the reviewer's verdict is "Needs changes" or "Block", treat
   ...
   per issue, each with `agent: "builder"`, a 1–5-word `taskName` summarising the fix,
   and a `task` that quotes the reviewer's issue verbatim and instructs the builder to fix
   it. Include an instruction to commit before finishing. Wait for one group to finish
