...
are disabled. General web searches use Google, Qwant, Seznam, Yandex, and Yep.
SearXNG automatically isolates unhealthy providers: generic 5xx, connection, and
timeout failures trigger a 15-minute cooldown, rate limits trigger one hour, and
CAPTCHAs or access denial trigger 24 hours. The Brave, DuckDuckGo, Google CSE,
and Startpage families are removed because their anonymous endpoints
persistently return CAPTCHAs, parser errors, or traffic blocks. Search requests
still go to
...
- Malformed or empty JSON: run `smoke.sh`; the extension reports malformed,
  ...
  separately.
- Upstream failures: affected engines are retried automatically after their
  cooldown. Transient network/server failures wait 15 minutes, rate limits wait
  one hour, and CAPTCHA/access denial waits 24 hours. Pi serializes concurrent
  search tool calls so one model turn cannot send an upstream burst.
- Partial-failure labels: Pi only labels a search partial when more than half of
  the providers observed in that response failed. Individual engine warnings
  remain in expanded output for diagnosis.
